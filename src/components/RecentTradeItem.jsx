  ...
  return (
    <div className="h-20 shadow-md rounded-md m-2 grid-cols-10 grid hover:shadow-lg cursor-pointer">
      <div className="col-span-7 flex flex-col ml-3">
        {profit ? (
          <span className="text-lg text-green-600 flex">
            {percentage}%<ArrowUp weight="fill" />
          </span>
        ) : (
          <span className="text-lg text-red-600 flex">
            {percentage}%<ArrowDown weight="fill" />
          </span>
        )}
        ...
        <span>
          {item.contract}{" "}
          {item.type == "Long" ? (
            <span className="border-red-600 border-2 p-1 text-red-600">
              Long
            </span>
          ) : (
            <span className="border-blue-600 border-2 p-1 text-blue-600">
              Short
            </span>
          )}{" "}
        </span>
